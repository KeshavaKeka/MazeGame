...
//            for (int y = 0; y < height; y++)
//            {
//                if (maze[x, y] == 1 && HasAdjacentWall(x, y) && Random.value > 0.6f) // Increase this value for fewer solo blocks
//                {
//                    maze[x, y] = 1;
//                }
//                else
...
//        if (cell.x > 2 && maze[cell.x - 3, cell.y] == 1)
//            neighbors.Add(new Vector2Int(cell.x - 3, cell.y));
//        if (cell.x < width - 3 && maze[cell.x + 3, cell.y] == 1)
//            neighbors.Add(new Vector2Int(cell.x + 3, cell.y));
//        if (cell.y > 2 && maze[cell.x, cell.y - 3] == 1)
//            neighbors.Add(new Vector2Int(cell.x, cell.y - 3));
//        if (cell.y < height - 3 && maze[cell.x, cell.y + 3] == 1)
//            neighbors.Add(new Vector2Int(cell.x, cell.y + 3));
...
//    public float density = 0.5f; // Density variable to control the wall density
...
//            for (int y = 0; y < height; y++)
//            {
//                if (maze[x, y] == 1 && HasAdjacentWall(x, y) && Random.value > density) // Adjust density
//                {
//                    maze[x, y] = 1;
//                }
//                else
...
public class MazeGenerator : MonoBehaviour
{
    ...
    void GenerateMaze()
    {
        ...
        // Adjust maze density and remove solo blocks
        for (int x = 0; x < width; x++)
        {
            for (int y = 0; y < height; y++)
            {
                if (maze[x, y] == 1 && Random.value > density)
                {
                    if (IsSoloBlock(x, y))
                    {
                        maze[x, y] = 0; // Remove solo blocks
                    }
                }
            }
        }
    ...
    bool IsSoloBlock(int x, int y)
    {
        // A block is a solo block if it has walls on all four sides
        return (x > 0 && maze[x - 1, y] == 1) &&
               (x < width - 1 && maze[x + 1, y] == 1) &&
               (y > 0 && maze[x, y - 1] == 1) &&
               (y < height - 1 && maze[x, y + 1] == 1);
